...
def table(header, data):
    if len(data) == 0:
        raise FormatError("data has no length")

    if len(header) != len(data[0]):
        raise FormatError("columns in header and data do not match")
    ...
    for register in registers:
        ...
        for field in register.field:
            if field.bitWidth == 1:
                ...
                output.write('- Bits {}:{} ({}) - {}\n'.format(field.bitOffset + field.bitWidth - 1, field.bitOffset, field.access, field.name))
...
def main():
    ...
    for peripheral in device.peripheral:
        ...
        for cluster in peripheral.cluster:
            ...
            format_registers(output, '{}.{}'.format(peripheral.name, cluster.name), cluster.register)

        format_registers(output, peripheral.name, peripheral.register)
